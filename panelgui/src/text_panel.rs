...
impl TextPanel {
    pub fn new(text: Text) -> winrt::Result<Self> {
    ...
    fn redraw_text(&self) -> winrt::Result<()> {
        if let Some(ref surface) = self.surface {
            ...
            let text: String = self.text.text.clone().into();
            ...
            let color = if self.text.enabled {
                self.text.color.clone()
            } else {
                Colors::gray()?
            };
